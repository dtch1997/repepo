...
def _mean_pairwise_cosine_agg(diff_vecs: list[Tensor]) -> float:
    ...
    pairwise_cosine_sims = pairwise_cosine_similarity(diff_vecs_tensor, diff_vecs_tensor)
    return torch.mean(pairwise_cosine_sims).item()
    
    # cosine_sims = []
    # for i, diff_vec_i in enumerate(diff_vecs):
    #     for j, diff_vec_j in enumerate(diff_vecs):
    #         if i != j:
    #             cosine_sims.append(
    #                 torch.nn.functional.cosine_similarity(diff_vec_i, diff_vec_j, dim=-1)
    #             )
    # return torch.mean(torch.stack(cosine_sims)).item()

def _cosine_agg(diff_vecs: list[Tensor]) -> float:
    """ Compute the cosine similarity between each difference vector and the mean. """
    mean = _mean_agg(diff_vecs)
    cosine_sims = [
...
def _mean_pairwise_dot_product_agg(diff_vecs: list[Tensor]) -> float:
    ...
    diff_vecs_tensor = torch.stack(diff_vecs)
    pairwise_dot_products = torch.einsum("id,jd->ij", diff_vecs_tensor, diff_vecs_tensor)
