...
model_full_name = get_model_full_name(model)
...
def make_id_vs_ood_steerability_plot_for_df(df, ax):
    ...
    print(
        f"Spearman correlation for SYS_POS -> USER_NEG: {spearman_corr} (p = {spearman_p})"
    )
...
def plot_qwen_and_llama_steerability_iid_vs_od(
    ...
    fig.tight_layout()
    fig.savefig(
        f"figures/qwen_and_llama2_steerability_id_vs_ood.pdf", bbox_inches="tight"
    )


plot_qwen_and_llama_steerability_iid_vs_od(llama_steerability_df, qwen_steerability_df)

# %%
# Correlate ID and OOD steerability between models
from scipy.stats import spearmanr

combined_df = llama_steerability_df.merge(
    qwen_steerability_df, on="dataset_name", suffixes=("_llama7b", "_qwen")
)
...
spearman_corr, spearman_p = spearmanr(
    combined_df["BASE -> BASE_llama7b"], combined_df["BASE -> BASE_qwen"]
)
...
sns.regplot(
    data=combined_df,
    x="SYS_POS -> USER_NEG_llama7b",
    y="SYS_POS -> USER_NEG_qwen",
    ax=ax,
)
# Print the coefficient
spearman_corr, spearman_p = spearmanr(
    combined_df["SYS_POS -> USER_NEG_llama7b"], combined_df["SYS_POS -> USER_NEG_qwen"]
)
print(
    f"Spearman correlation for SYS_POS -> USER_NEG: {spearman_corr} (p = {spearman_p})"
)
...
combined_steerability_variance_df = llama_steerability_variance_df.merge(
    qwen_steerability_variance_df, on="dataset_name", suffixes=("_llama7b", "_qwen")
)
...
def plot_qwen_and_llama_steerability_variance_iid_vs_od(
    ...
    fig.tight_layout()
    fig.savefig(
        f"figures/qwen_and_llama2_steerability_variance_id_vs_ood.pdf",
        bbox_inches="tight",
    )
...
def plot_qwen_and_llama_steerability_variance_id_vs_ood(
    ...
    ax.axvline(0, color="black", linestyle="-")
    sns.regplot(
        data=combined_df, x="BASE -> BASE_llama7b", y="BASE -> BASE_qwen", ax=ax
    )
    spearman_corr, spearman_p = spearmanr(
        combined_df["BASE -> BASE_llama7b"], combined_df["BASE -> BASE_qwen"]
    )
    ...
    # Print the coefficient
    spearman_corr, spearman_p = spearmanr(
        combined_df["SYS_POS -> USER_NEG_llama7b"],
        combined_df["SYS_POS -> USER_NEG_qwen"],
    )
    print(
        f"Spearman correlation for SYS_POS -> USER_NEG: {spearman_corr} (p = {spearman_p})"
    )
    ...
    fig.savefig(
        f"figures/correlation_steerability_variance_between_models.pdf",
        bbox_inches="tight",
    )


plot_qwen_and_llama_steerability_variance_id_vs_ood(combined_steerability_variance_df)
...
def plot_steerability_vs_variance(
    ...
    fig, axs = plt.subplots(nrows=1, ncols=2, figsize=(9, 3.5), sharey=True)
    llama_ood_df = llama_steerability_df.merge(
        llama_steerability_variance_df,
        on="dataset_name",
        suffixes=("_steerability", "_variance"),
    )
    ...
    sns.regplot(
        data=llama_ood_df,
        x="BASE -> BASE_steerability",
        y="BASE -> BASE_variance",
        ax=ax,
    )
    sns.regplot(
        data=llama_ood_df,
        x="SYS_POS -> USER_NEG_steerability",
        y="SYS_POS -> USER_NEG_variance",
        ax=ax,
    )
    sns.regplot(
        data=llama_ood_df,
        x="BASE -> USER_NEG_steerability",
        y="BASE -> USER_NEG_variance",
        ax=ax,
    )
    sns.regplot(
        data=llama_ood_df,
        x="BASE -> USER_POS_steerability",
        y="BASE -> USER_POS_variance",
        ax=ax,
    )
    sns.regplot(
        data=llama_ood_df,
        x="SYS_NEG -> USER_POS_steerability",
        y="SYS_NEG -> USER_POS_variance",
        ax=ax,
    )
    ...
    qwen_ood_df = qwen_steerability_df.merge(
        qwen_steerability_variance_df,
        on="dataset_name",
        suffixes=("_steerability", "_variance"),
    )
    ax = axs[1]
    ...
    sns.regplot(
        ...
        y="SYS_POS -> USER_NEG_variance",
        ax=ax,
        label=r"SYS_POS $\rightarrow$ USER_NEG",
    )
    sns.regplot(
        data=qwen_ood_df,
        x="BASE -> USER_NEG_steerability",
        y="BASE -> USER_NEG_variance",
        ax=ax,
        label=r"BASE $\rightarrow$ USER_NEG",
    )
    sns.regplot(
        data=qwen_ood_df,
        x="BASE -> USER_POS_steerability",
        y="BASE -> USER_POS_variance",
        ax=ax,
        label=r"BASE $\rightarrow$ USER_POS",
    )
    sns.regplot(
        data=qwen_ood_df,
        x="SYS_NEG -> USER_POS_steerability",
    ...
    fig.savefig(
        "figures/qwen_and_llama_steerability_vs_variance.pdf", bbox_inches="tight"
    )
...
def compute_steerability_variance_id_vs_ood(df):
    ...
    fig.tight_layout()
    fig.savefig(
        f"figures/{model}_steerability_variance_id_vs_ood.pdf", bbox_inches="tight"
    )
