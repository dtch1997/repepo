...
def compute_steerability_id_vs_ood(df):
    ...
    sns.lineplot(data=steerability_df, x='BASE -> BASE', y='BASE -> BASE', color='black', linestyle='--')
    ax.axhline(0, color='black', linestyle='--')
    ax.axvline(0, color='black', linestyle='--')

    # ID vs OOD
    def make_plot_for_dist_shift(dist_shift: str):
        sns.regplot(data=steerability_df, x='BASE -> BASE', y=dist_shift, scatter = False, ax = ax)
        sns.scatterplot(data=steerability_df, x='BASE -> BASE', y=dist_shift, label=dist_shift, ax = ax)
...
def compute_steerability_variance_id_vs_ood(df):
    ...
    # Calculate overall steerability by dataset. 
    # Calculate steerability within each flavour
    var_slope = df.groupby(['dataset_name', 'steering_label', 'dataset_label'])['slope'].var()
    df = df.merge(var_slope, on=['dataset_name', 'steering_label', 'dataset_label'], suffixes=('', '_var'))
    ...
    steerability_id_df = steerability_id_df.rename(columns={'slope_var': 'steerability'})
    ...
    # Rename 'slope_var' to 'steerability'
    steerability_ood_df = steerability_ood_df.rename(columns={'slope_var': 'steerability'})
    ...
    # Rename 'slope_var' to 'steerability'
    steerability_base_to_user_neg_df = steerability_base_to_user_neg_df.rename(columns={'slope_var': 'steerability_base_to_user_neg'})
    ...
    # Rename 'slope_var' to 'steerability'
    steerability_base_to_user_pos_df = steerability_base_to_user_pos_df.rename(columns={'slope_var': 'steerability_base_to_user_pos'})
    ...
    # Rename 'slope_var' to 'steerability'
    steerability_ood_to_user_neg_df = steerability_ood_to_user_neg_df.rename(columns={'slope_var': 'steerability_ood_to_user_neg'})
    ...
    # Rename 'slope_var' to 'steerability'
    steerability_ood_to_user_pos_df = steerability_ood_to_user_pos_df.rename(columns={'slope_var': 'steerability_ood_to_user_pos'})
