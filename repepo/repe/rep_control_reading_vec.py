...
class WrappedReadingVecModel(torch.nn.Module):
    ...
    def wrap_gptneox_post_attention_layernorm(self, layer_id):
        ...
        if not self.is_wrapped(block):
            self.model.gpt_neox.layers[layer_id].post_attention_layernorm = WrappedBlock(block)
    ...
    def set_controller(self, layer_ids, activations, block_name='decoder_block', token_pos=None, masks=None, normalize=False):
        ...
        def _set_controller(layer_id, activations, block_name, masks, normalize):
            current_layer = self.model.model.layers[layer_id] if not self.is_gpt_neox else self.model.gpt_neox.layers[layer_id]
            ...
            if block_name == 'decoder_block':
                ...
                current_block = current_layer.block
                # if self.is_gpt_neox:
                if block_name == 'attention' and self.is_wrapped(current_block.self.attn):
                    current_block.attention.set_controller(activations, token_pos, masks, normalize)
                # else:  
                if block_name == 'self_attn' and self.is_wrapped(current_block.self_attn):
    ...
    def reset(self):
        # TODO: clean up
        layers_iter = self.model.gpt_neox.layers if self.is_gpt_neox else self.model.model.layers
    ...
    def unwrap(self):
        ...
        if self.is_gpt_neox:
            for l, layer in enumerate(self.model.gpt_neox.layers):
                ...
                if self.is_wrapped(layer.attention):
                    self.model.gpt_neox.layers[l].attention = self.model.gpt_neox.layers[l].attention.block
                if self.is_wrapped(layer.mlp):
                    self.model.gpt_neox.layers[l].mlp = self.model.gpt_neox.layers[l].mlp.block
                if self.is_wrapped(layer.input_layernorm):
                    self.model.gpt_neox.layers[l].input_layernorm = self.model.gpt_neox.layers[l].input_layernorm.block
                if self.is_wrapped(layer.post_attention_layernorm):
                    self.model.gpt_neox.layers[l].post_attention_layernorm = self.model.gpt_neox.layers[l].post_attention_layernorm.block
