...
devbox/%:
	...
	python -c "print(open('k8s/devbox.yaml').read().format(NAME='${DEVBOX_NAME}', IMAGE='${PROJECT_URL}:${RELEASE_PREFIX}-$*', COMMIT_FULL='${COMMIT_FULL}', CPU='${CPU}', MEMORY='${MEMORY}', GPU='${GPU}', USER_ID=${DEVBOX_UID}, GROUP_ID=${DEVBOX_UID}, WANDB_PROJECT=${IMAGE_NAME}))" | kubectl create -f -
devbox: devbox/main
	true
